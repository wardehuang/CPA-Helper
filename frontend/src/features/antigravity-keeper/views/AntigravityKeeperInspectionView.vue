...
import {
  clearAntigravityKeeperLogs,
  getAntigravityKeeperSettings,
  getAntigravityKeeperStatus,
  listAntigravityKeeperAccounts,
  previewAntigravityKeeperSchedule,
  runAntigravityKeeperOnce,
  startAntigravityKeeper,
  stopAntigravityKeeper,
  updateAntigravityKeeperSettings,
...
const status = ref<AntigravityKeeperStatus | null>(null)
...
function applySettings(nextSettings: Awaited<ReturnType<typeof getAntigravityKeeperSettings>>) {
...
async function loadAll() {
  ...
  try {
    ...
    console.log('--- Antigravity Inspection View APIs ---')
    console.log('getAntigravityKeeperSettings:', settings)
    console.log('getAntigravityKeeperStatus:', nextStatus)
    console.log('listAntigravityKeeperAccounts:', accountResponse)
    console.log('----------------------------------------')
...
function normalizedRules(): AntigravityKeeperPriorityRule[] {
...
async function saveSettings() {
  ...
  isSaving.value = true
  try {
    const payload: AntigravityKeeperSettingsUpdatePayload = {
      ...form,
      priority_rules: rules,
    }
    const saved = await updateAntigravityKeeperSettings(payload)
    applySettings(saved)
...
async function loadSchedulePreview() {
  ...
  try {
    const preview = await previewAntigravityKeeperSchedule({ schedule_cron: scheduleCron })
...
function removeRule(rule: AntigravityKeeperPriorityRule) {
  ...
  <section class="page inspection-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">{{ t('维护 Antigravity auth file 的健康状态和调度优先级', 'Maintain Antigravity auth file health and scheduling priorities') }}</p>
      </div>
      <NSpace>
    ...
    <div class="inspection-settings-grid">
          ...
          <div class="section-heading">
            ...
            <NSpace class="config-actions" size="small">
              <NButton
                ...
                @click="runAction(runAntigravityKeeperOnce, t('已开始执行一轮', 'Started one inspection run'))"
              >
                {{ t('执行一轮', 'Run Once') }}
              </NButton>
              <NButton
                ...
                @click="runAction(startAntigravityKeeper, t('已开始自动巡检', 'Automatic inspection started'))"
              >
              ...
              <NButton
                ...
                :loading="isActing"
                :disabled="!isDaemonRunning"
                @click="runAction(stopAntigravityKeeper, t('已请求停止', 'Stop requested'))"
              >
                {{ t('停止', 'Stop') }}
              </NButton>
            </NSpace>
      ...
      <div class="panel-inner log-panel-inner">
        <div class="section-heading">
          ...
          <NSpace class="log-actions" size="small">
            ...
            <NButton secondary :loading="isActing" @click="runAction(clearAntigravityKeeperLogs, t('日志已清空', 'Logs cleared'))">
              <template #icon>
