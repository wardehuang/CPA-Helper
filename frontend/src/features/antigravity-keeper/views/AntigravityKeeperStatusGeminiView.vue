...
import {
  bulkDeleteAntigravityKeeperAccounts,
  deleteAntigravityKeeperAccount,
  disableAntigravityKeeperAccount,
  enableAntigravityKeeperAccount,
  getAntigravityKeeperStatus,
  getAntigravityKeeperSettings,
  listAntigravityKeeperAccounts,
  refreshAntigravityKeeperAccounts,
  updateAntigravityKeeperPriority,
} from '@/features/antigravity-keeper/api/antigravityKeeperApi'
import type {
  AntigravityKeeperAccount,
  AntigravityKeeperPriorityRule,
  AntigravityKeeperQuotaWindowUsage,
...
type QuotaWindowItem = {
  ...
  resetAt: string | null
  usage: AntigravityKeeperQuotaWindowUsage | null
}
...
const ACCOUNT_STATUS_PREFERENCE_STORAGE_KEY = 'cpa-helper-antigravity-keeper-status-preferences'
...
const accounts = ref<AntigravityKeeperAccount[]>([])
const priorityRules = ref<AntigravityKeeperPriorityRule[]>([])
const keeperStatus = ref<AntigravityKeeperStatus | null>(null)
const selectedAccount = ref<AntigravityKeeperAccount | null>(null)
...
const priorityDialog = reactive({
  ...
  account: null as AntigravityKeeperAccount | null,
...
function pagedAccounts(source: AntigravityKeeperAccount[], page: number): AntigravityKeeperAccount[] {
  if (accountDisplaySize.value === 'all') {
    return source
  }
...
function matchesPriorityFilter(account: AntigravityKeeperAccount, value: PriorityFilter): boolean {
...
function matchesStatusFilter(account: AntigravityKeeperAccount, value: AccountStatusFilter): boolean {
  if (value === 'enabled') {
...
function hasAccountError(account: AntigravityKeeperAccount): boolean {
...
function accountPriority(account: AntigravityKeeperAccount): number {
  return account.priority ?? 0
}

function isQuotaExhaustedAccount(account: AntigravityKeeperAccount): boolean {
...
function normalAccountTypePriority(account: AntigravityKeeperAccount): number {
...
function compareNormalAccounts(left: AntigravityKeeperAccount, right: AntigravityKeeperAccount): number {
...
function sortAccountsForDisplay(
  source: AntigravityKeeperAccount[],
  defaultCompare?: (left: AntigravityKeeperAccount, right: AntigravityKeeperAccount) => number,
): AntigravityKeeperAccount[] {
  const rows = [...source]
...
function compareAccountsByActiveSort(left: AntigravityKeeperAccount, right: AntigravityKeeperAccount): number {
...
function compareAccountFileName(left: AntigravityKeeperAccount, right: AntigravityKeeperAccount): number {
  return left.name.localeCompare(right.name)
}

function defaultPriority(account: AntigravityKeeperAccount): number | null {
  if (!account.account_type) {
    return null
  }
...
function quotaWindowSecondsFor(account: AntigravityKeeperAccount, window: 'primary' | 'secondary'): number | null {
  if (window === 'primary') {
...
function isPaidQuotaWindow(account: AntigravityKeeperAccount): boolean {
  return (
...
function isFreeQuotaWindow(account: AntigravityKeeperAccount): boolean {
  return (
...
function quotaWindowLabels(account: AntigravityKeeperAccount): { primary: string; secondary: string } {
  if (isFreeQuotaWindow(account)) {
    return { primary: t('周限额', 'Weekly Limit'), secondary: t('周限额', 'Weekly Limit') }
  }
  if (isPaidQuotaWindow(account)) {
    return { primary: t('5小时限额', '5-Hour Limit'), secondary: t('周限额', 'Weekly Limit') }
  }
  return { primary: t('限额', 'Limit'), secondary: t('限额', 'Limit') }
}

function shouldShowQuotaWindow(account: AntigravityKeeperAccount): boolean {
  return !account.disabled
}

function quotaWindowItems(account: AntigravityKeeperAccount): QuotaWindowItem[] {
  ...
  return [{
      ...
      remainingPercent: nullableRemainingQuotaPercent(account.secondary_used_percent),
      resetAt: account.secondary_reset_at,
      usage: account.secondary_window_usage,
  }]
}

function quotaSortRemainingPercent(account: AntigravityKeeperAccount, key: AccountSortKey): number | null {
  return shouldShowQuotaWindow(account) ? nullableRemainingQuotaPercent(account.secondary_used_percent) : null
}
...
function quotaBarTone(percent: number | null): string {
  if (percent === null) {
    return 'is-unknown'
  }
  if (percent < 30) {
...
function quotaProgressPercent(item: QuotaWindowItem): number {
  return item.remainingPercent ?? 0
}

function quotaRemainingText(item: QuotaWindowItem): string {
  return item.remainingPercent === null
    ? t('额度未知', 'Quota unknown')
    : t(`剩余 ${item.remainingPercent}%`, `${item.remainingPercent}% remaining`)
}

function quotaRemainingValueText(item: QuotaWindowItem): string {
  return item.remainingPercent === null ? '-' : `${item.remainingPercent}%`
}
...
function quotaText(account: AntigravityKeeperAccount): string {
  ...
  return items
    .map((item) => {
      ...
      return resetTime
        ? t(
            `${item.label}${quotaRemainingText(item)}，刷新 ${resetTime}，${usageText}`,
            `${item.label} ${quotaRemainingText(item)}, refreshes ${resetTime}, ${usageText}`,
          )
        : t(
            `${item.label}${quotaRemainingText(item)}，${usageText}`,
            `${item.label} ${quotaRemainingText(item)}, ${usageText}`,
          )
    })
    .join(' / ')
...
function quotaWindowUsageTags(item: QuotaWindowItem): QuotaUsageTag[] {
  ...
  return [
    { label: t('请求', 'Requests'), value: formatInteger(usage.records) },
    { label: 'Tokens', value: formatCompact(usage.total_tokens) },
    { label: t('费用', 'Cost'), value: formatUsd(usage.estimated_cost_usd) },
  ]
}

function quotaWindowResetText(item: QuotaWindowItem): string {
  const resetTime = formatQuotaResetTime(item.resetAt)
  return resetTime ? t(`刷新 ${resetTime}`, `Refreshes ${resetTime}`) : t('官方未返回刷新时间', 'Refresh time not returned by upstream')
}
...
function quotaWindowUsageTone(item: QuotaWindowItem): string {
  return item.usage?.stale === true ? 'is-stale' : ''
}

function latestActionText(account: AntigravityKeeperAccount): string {
  const text = account.last_error?.trim() || account.latest_action?.trim()
  return text ? serverText(text, '账号状态', 'Account status') : '-'
}

function disabledCardErrorText(account: AntigravityKeeperAccount): string {
  if (!account.disabled) {
    return ''
  }
  const text =
...
function disabledStatusCodeText(account: AntigravityKeeperAccount): string | null {
...
function disabledStatusCodeTitle(account: AntigravityKeeperAccount): string | null {
  const text = disabledStatusCodeText(account)
  return text === null ? null : `HTTP ${text}`
}

function renderQuotaCell(account: AntigravityKeeperAccount) {
  const items = quotaWindowItems(account)
  if (items.length === 0) {
    return '-'
  }
    ...
    items.map((item) => {
      ...
      return h(
        'div',
        {
          ...
          title: resetTime
            ? t(
                `${item.label}${quotaRemainingText(item)}，刷新 ${resetTime}；${quotaWindowUsageTitle(item)}`,
                `${item.label} ${quotaRemainingText(item)}, refreshes ${resetTime}; ${quotaWindowUsageTitle(item)}`,
              )
            : t(
                `${item.label}${quotaRemainingText(item)}；${quotaWindowUsageTitle(item)}`,
                `${item.label} ${quotaRemainingText(item)}; ${quotaWindowUsageTitle(item)}`,
              ),
        },
        [
          h('div', { class: 'quota-window-head' }, [
            ...
            h('span', { class: 'quota-window-meta' }, [
              h('span', { class: 'quota-window-percent' }, quotaRemainingValueText(item)),
          ...
          h('div', { class: 'quota-window-track' }, [
            h('div', {
              ...
              style: { width: `${quotaProgressPercent(item)}%` },
            }),
          ]),
        ],
      )
...
function renderQuotaUsageCell(account: AntigravityKeeperAccount) {
  const items = quotaWindowItems(account)
  if (items.length === 0) {
    return '-'
  }
...
function renderAccountIdentityCell(account: AntigravityKeeperAccount) {
...
function renderAccountTypeCell(account: AntigravityKeeperAccount) {
...
function renderAccountPriorityCell(account: AntigravityKeeperAccount) {
...
function renderLastCheckedCell(account: AntigravityKeeperAccount) {
...
function renderLatestActionCell(account: AntigravityKeeperAccount) {
...
async function loadKeeperStatus() {
  try {
    keeperStatus.value = await getAntigravityKeeperStatus()
  } catch {
    return
  }
}

function accountRowKey(account: AntigravityKeeperAccount): string {
  return account.name
}
...
function isRefreshAccountSelected(account: AntigravityKeeperAccount): boolean {
  return selectedRefreshAccountNameSet.value.has(account.name)
}

function toggleRefreshAccountSelection(account: AntigravityKeeperAccount) {
...
function handleAccountCardClick(account: AntigravityKeeperAccount) {
  if (refreshSelectMode.value) {
...
function accountTableRowProps(account: AntigravityKeeperAccount) {
...
async function submitBulkDelete() {
  ...
  try {
    const result = await bulkDeleteAntigravityKeeperAccounts({ auth_names: authNames })
...
function openDetail(account: AntigravityKeeperAccount) {
  selectedAccount.value = account
  detailOpen.value = true
}

function openPriorityDialog(account: AntigravityKeeperAccount) {
...
async function submitPriorityDialog() {
  ...
  await runAccountAction(
    ...
    'priority',
    () => updateAntigravityKeeperPriority(priorityDialog.account!.name, value),
...
function confirmEnableAccount(account: AntigravityKeeperAccount) {
  openAccountConfirm(
...
function confirmDisableAccount(account: AntigravityKeeperAccount) {
  openAccountConfirm(
...
function confirmDeleteAccount(account: AntigravityKeeperAccount) {
  openAccountConfirm(
...
function enableAccount(account: AntigravityKeeperAccount) {
  return runAccountAction(
    account,
    'toggle',
    () => enableAntigravityKeeperAccount(account.name),
    t('账号已启用', 'Account enabled'),
  )
}

function disableAccount(account: AntigravityKeeperAccount) {
  return runAccountAction(
    account,
    'toggle',
    () => disableAntigravityKeeperAccount(account.name),
    t('账号已禁用', 'Account disabled'),
  )
}

function deleteAccount(account: AntigravityKeeperAccount) {
  return runAccountAction(
    account,
    'delete',
    () => deleteAntigravityKeeperAccount(account.name),
    t('账号已删除', 'Account deleted'),
  )
}

function refreshAccount(account: AntigravityKeeperAccount, options: { closeDetail?: boolean } = {}) {
...
async function pollRefreshUntilIdle() {
  ...
  for (;;) {
    ...
    try {
      const status = await getAntigravityKeeperStatus()
...
async function refreshAccounts(
  ...
  const refreshKeys = authNames
    ...
    .filter((account): account is AntigravityKeeperAccount => account !== undefined)
  ...
  try {
    await refreshAntigravityKeeperAccounts({ auth_names: authNames })
...
function accountActionKey(account: AntigravityKeeperAccount, action: AccountAction): string {
  return `${action}\u0000${account.name}`
}

function isActionLoading(account: AntigravityKeeperAccount, action: AccountAction): boolean {
  return actingActions.value.has(accountActionKey(account, action))
}

function isRowActing(account: AntigravityKeeperAccount): boolean {
...
async function runAccountAction(
  account: AntigravityKeeperAccount,
...
const baseColumns = computed<DataTableColumns<AntigravityKeeperAccount>>(() => [
  {
    title: t('账号', 'Account'),
    key: 'identity',
    width: 360,
...
const disabledBaseColumns = computed<DataTableColumns<AntigravityKeeperAccount>>(
  () => baseColumns.value.filter(
    (column) => !('key' in column) || (column.key !== 'quota' && column.key !== 'quota_usage'),
  ),
)

const disabledActionColumn = computed<DataTableColumns<AntigravityKeeperAccount>[number]>(() => ({
  title: '',
  key: 'actions',
  width: 224,
  fixed: 'right',
  render: (row: AntigravityKeeperAccount) => {
    return h(
      NSpace,
...
const normalActionColumn = computed<DataTableColumns<AntigravityKeeperAccount>[number]>(() => ({
  title: '',
  key: 'actions',
  width: 232,
  fixed: 'right',
...
const disabledColumns = computed<DataTableColumns<AntigravityKeeperAccount>>(() => [
  {
    type: 'selection',
    width: 44,
    disabled: (row: AntigravityKeeperAccount) => isRowActing(row) || isBulkDeleting.value,
  },
  ...disabledBaseColumns.value,
  disabledActionColumn.value,
])

const normalColumns = computed<DataTableColumns<AntigravityKeeperAccount>>(() => [
  ...baseColumns.value,
                  ...
                  <template v-if="isBarCardView">
                    ...
                    >
                      <div class="card-quota-head">
                        ...
                        <strong>{{ quotaRemainingText(item) }}</strong>
                      </div>
                    ...
                    >
                      <div class="card-quota-ring-head">
                        <div
                          class="quota-ring"
                          :class="quotaBarTone(item.remainingPercent)"
                          :style="{ '--quota-deg': `${quotaProgressPercent(item) * 3.6}deg` }"
                        >
                          <span>{{ quotaRemainingValueText(item) }}</span>
                        </div>
