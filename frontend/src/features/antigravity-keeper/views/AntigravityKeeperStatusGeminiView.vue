...
function displayAccountName(account: AntigravityKeeperAccount): string {
  const source = account.email?.trim() || account.name
  return source.split('@')[0] || source
}

function accountStatusLabel(account: AntigravityKeeperAccount): string {
  if (account.disabled) {
...
function renderAccountStatusCell(account: AntigravityKeeperAccount) {
...
const accountActionColumn = computed<DataTableColumns<AntigravityKeeperAccount>[number]>(() => ({
  title: t('操作', 'Actions'),
  key: 'actions',
  width: 232,
...
const accountColumns = computed<DataTableColumns<AntigravityKeeperAccount>>(() => [
  {
    title: t('账号', 'Account'),
    key: 'identity',
    width: 220,
